...
interface WritingContent {
  ...
  minimumWords: number;
  maximumWords: number;
  points: number; // Stored from DB, but we will override to 25
}
...
interface WritingEvaluationResult {
  ...
  score: number; // Dynamically computed out of 25
  critique: string;
  improvements: string;
  issuesFound: string[];
  wordCount: number;
...
const WritingQuiz = ({ exercise }: WritingQuizProps) => {
  ...
  const q = contentList[0]; // Writing assignments always contain exactly 1 challenge item
  ...
  // --- Fixed Dynamic Assessment Rules ---
  const MAX_POINTS = 25;

  // Track real-time string word formatting count
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          handleSubmitWriting(); // Auto-submit when countdown hits zero
          return 0;
        }
        return prev - 1;
      });
  ...
  // Live Grammar, Spelling, and Punctuation Evaluator Mechanics
  const handleSubmitWriting = async () => {
    ...
    try {
      ...
      // ==========================================
      // 1. NEW: DYNAMIC TOPIC RELEVANCY CHECKER
      // ==========================================
      const lowerText = trimmedText.toLowerCase();

      // Simple rule-based dictionary maps based on database keywords
      ...
      // Automatically detect which category to test against based on the topic prompt text
      let assignedCategory = "family"; // fallback default
      if (q.topic.toLowerCase().includes("family")) assignedCategory = "family";
      else if (
        q.topic.toLowerCase().includes("hobby") ||
        q.topic.toLowerCase().includes("hobbies")
      )
        assignedCategory = "hobby";
      else if (
        q.topic.toLowerCase().includes("routine") ||
        q.topic.toLowerCase().includes("day")
      )
        assignedCategory = "routine";

      const targetWords = topicKeywords[assignedCategory];

      // Count how many unique target vocabulary words are present in the text
      const matchedWords = targetWords.filter((word) => {
        // Use regex bounds (\b) so "parents" matches but "transparent" doesn't accidentally trigger
        const regex = new RegExp(`\\b${word}\\b`, "i");
        return regex.test(lowerText);
      });

      // Strategy: For a family topic, they must use at least 3 unique family-related words
      const MINIMUM_REQUIRED_MATCHES = 3;

      if (matchedWords.length < MINIMUM_REQUIRED_MATCHES) {
        // Impose a severe flat penalty for writing completely off-topic material
        penalty += 15;
        issues.push(
      ...
      // ==========================================
      // 2. STANDARD GRAMMAR CHECK (LanguageTool)
      // ==========================================
      ...
      matches.forEach((match: any) => {
        ...
        const errorSnippet = trimmedText.substring(
          match.offset,
          match.offset + match.length,
        );
        const suggestion = match.replacements
          .slice(0, 2)
          .map((r: any) => r.value)
          .join(" / ");
        issues.push(
      ...
      // Final score calculation clamped to a minimum of 0
      ...
      const submissionPayload = {
        ...
        category: "writing", // Explicitly change this flag to "speaking" in your other file
      ...
      const backendResponse = await fetch("/api/submissions/save", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          // Pass authorization tokens if required by your web server auth configuration
      ...
      if (!backendResponse.ok) {
        const errorDetails = await backendResponse.json();
        throw new Error(errorDetails.message || "Failed to preserve grading record.");
      }

      setShowResults(true);
    } catch (err) {
      console.error("Syntax evaluation exception: ", err);
      alert("Error communicating with data validation pipelines.");
    } finally {
  ...
  const calculatedPercentage = evaluation
    ? (evaluation.score / MAX_POINTS) * 100
    : 0;
  const passed = calculatedPercentage >= (exercise.passing_percentage ?? 70);

  // --- RESULTS AND DIAGNOSTICS DISPLAY SCREEN ---
      ...
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center shadow-sm">
            <div
              className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 ${passed ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
            >
              {passed ? <CheckCircle2 size={44} /> : <X size={44} />}
            </div>
            <h2 className="text-3xl font-bold text-slate-800">
              {passed ? "Writing Approved!" : "Revision Recommended"}
            </h2>
            <p className="text-sm text-slate-400 mt-1">
              Required Benchmark Standard: {exercise.passing_percentage}%
            </p>
          ...
          <h3 className="text-lg font-bold text-slate-700 flex items-center gap-2">
            <Sparkles size={18} className="text-indigo-500" /> Structural
            Writing Quality Assessment Report
          </h3>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
            <div>
              ...
              <p className="text-sm text-slate-500 italic mt-1">
                Topic Directive: {q.topic}
              </p>
            </div>

            <div className="bg-slate-50 p-5 rounded-xl border border-slate-100">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">
                Your Submitted Entry Text Structure:
              </p>
              <p className="text-slate-700 whitespace-pre-wrap font-medium mt-1.5 leading-relaxed">
                "{evaluation.textSubmitted}"
              </p>
            </div>
            ...
            {evaluation.issuesFound.length > 0 && (
              <div className="bg-red-50/50 border border-red-100 p-4 rounded-xl space-y-1">
                <p className="text-xs font-bold text-red-800 uppercase tracking-wide flex items-center gap-1">
                  <ShieldAlert size={14} /> Grammar, Spelling, & Punctuation
                  Deviations Logged:
                </p>
                <ul className="list-none space-y-2 text-xs text-slate-600 mt-2">
                  {evaluation.issuesFound.map((issue, idx) => (
                    <li
                      key={idx}
                      className="bg-white p-2.5 rounded border border-red-100/50 shadow-sm leading-relaxed"
                    >
                      {issue}
                    </li>
                  ))}
      ...
      <div className="max-w-2xl w-full mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <span
              className={`text-sm font-mono font-bold px-3 py-1 rounded-md ${timeLeft < 60 ? "bg-red-50 text-red-600 animate-pulse" : "bg-slate-100 text-slate-700"}`}
            >
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-5">
          ...
          {/* Constraint Metadata Info Cards */}
          <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3.5 rounded-xl border border-slate-100 text-center text-xs text-slate-500 font-semibold">
            <div>
              ...
              <span className="text-slate-800 font-bold">
                {q.minimumWords} words
              </span>
            </div>
            <div>
              Target Upper Ceiling:{" "}
              <span className="text-slate-800 font-bold">
                {q.maximumWords} words
              </span>
            </div>
          </div>

          {/* Primary Textarea Writing Workspace Console */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">
              Composition Console Area
            </label>
            <textarea
          ...
          {/* Operational Word Counter Metrics Tracker Footer Row */}
          <div className="flex items-center justify-between pt-1 text-xs">
            <div
              className={`flex items-center gap-1 font-bold ${isOutOfRange && userInput.length > 0 ? "text-amber-600" : "text-emerald-600"}`}
            >
              {isOutOfRange && userInput.length > 0 && (
